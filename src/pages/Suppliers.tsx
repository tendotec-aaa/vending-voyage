    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="relative max-w-md">
          ...
          <Input placeholder={t('suppliers.searchSuppliers')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
        </div>

        {isLoading ? (
          <div className="text-muted-foreground">{t('suppliers.loadingSuppliers')}</div>
        ) : filteredSuppliers.length === 0 ? (
          <div className="text-center py-12">
            <Building2 className="w-12 h-12 mx-auto text-muted-foreground mb-4" />
            <h3 className="text-lg font-medium text-foreground mb-1">{searchQuery ? t('suppliers.noSuppliersFound') : t('suppliers.noSuppliersYet')}</h3>
            <p className="text-muted-foreground mb-4">{searchQuery ? t('suppliers.adjustSearch') : t('suppliers.addFirst')}</p>
            {!searchQuery && (
              <Button onClick={() => setIsFormOpen(true)}><Plus className="w-4 h-4 mr-2" /> {t('suppliers.addSupplier')}</Button>
            )}
          </div>
        ) : (
          <Card>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">{t('suppliers.leadTime')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
