      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => navigate("/sales/new")}>
            <Plus className="mr-2 h-4 w-4" /> {t('sales.newSale')}
          </Button>
        </div>
        ...
        <Card className="p-4">
          <div className="relative flex-1">
            ...
            <Input placeholder={t('sales.searchSales')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
          </div>
        </Card>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">{t('sales.noSales')}</h3>
            <p className="text-muted-foreground mb-4 max-w-sm">
              {searchQuery ? t('sales.adjustSearch') : t('sales.createFirst')}
            </p>
            <Button onClick={() => navigate("/sales/new")}>
              <Plus className="mr-2 h-4 w-4" /> {t('sales.createSale')}
            </Button>
          </div>
        ) : (
          <Card>
              ...
              <TableBody>
                {filteredSales.map((sale: any) => (
                  <TableRow key={sale.id} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/sales/${sale.id}`)}>
                    ...
                    <TableCell className="text-muted-foreground">{sale.sale_date ? new Date(sale.sale_date).toLocaleDateString() : "—"}</TableCell>
                    <TableCell className="text-muted-foreground">{(sale.sale_items || []).length}</TableCell>
                    <TableCell className="text-right font-medium text-foreground">${fmt2(sale.total_amount || 0)}</TableCell>
                    <TableCell>
                      <Badge variant={sale.paid ? "default" : "outline"}>
                        {sale.paid ? t('sales.paid') : t('sales.unpaid')}
                      </Badge>
                    </TableCell>
                  </TableRow>
                ))}
